...
def tot_grade() -> float:
    return lab_wgr + quiz_wgr + asgn_wgr + midterms_wgr + final_wgr + mid_final_prep_wgr
...
average_midterms = float(midterm2 + midterm1) / 2
midterms_wgr = grading(average_midterms, 25)
mid_final_prep_wgr = grading(mid_and_final, 6)
final_wgr = grading(Final, 18)

print('Your finale grade for the course :', str(tot_grade())[:6],'%')
